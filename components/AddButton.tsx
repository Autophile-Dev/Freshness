...
import ActionAddSimple from './ActionsIcons/ActionAddSimple'
// Type of variant
...
const generateSize = (size:Size): string => {
  switch(size){
    ...
    case "Medium":
      return "h-button-medium px-button-medium-row py-medium-large-column ";
    case "Small":
...
const AddButton = ({label, variant, size, type, className = ' '}:ButtonProps) => {
  if(type === "Default"){
    return (
    ...
    return(
      <button className={`${generateVariant(variant)} ${generateSize(size)} 
      ${className}`}><ActionAddSimple className={`${generateIconColor(variant)}
      ${generateIconSize()} order-none`} />{label}</button>
    ) 
  }
  else if(type === "Right"){
    return(
      <button className={`${generateVariant(variant)} ${generateSize(size)} 
      ${className}`}>{label}<ActionAddSimple className={`${generateIconColor(variant)}
      ${generateIconSize()} order-1`} /></button>
    )
  }
  else{
