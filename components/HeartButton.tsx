...
const HeartButton = ({label, variant, size, type, className = ' '}:ButtonProps) => {
  if(type === "Default"){
    return (
    ...
    return(
      <button className={`${generateVariant(variant)} ${generateSize(size)} 
      ${className}`}><ActionHeart className={`${generateIconColor(variant)}
    ...
    return(
      <button className={`${generateVariant(variant)} ${generateSize(size)} 
      ${className}`}>{label}<ActionHeart className={`${generateIconColor(variant)}
